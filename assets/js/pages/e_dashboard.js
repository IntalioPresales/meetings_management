...
$(function() {    
        ...
        d3.csv("assets/demo_data/dashboard/app_sales_heatmap.csv", function(error, data) {
            ...
            hourGroup
                .append("text")
                    ...
                    .text(function (d, i) { return d3.sum(d.values, function(d) { return d.value; }) + " person today" });
            ...
            hourGroup.each(function(d) {
                heatMap
                    .on("mouseover", function (d, i) {
                        ...
                        d3.select(this.parentNode).select('.sales-count').text(function(d) { return d.values[i].value + " person at " + formatTime(d.values[i].date); })
                    })
                    .on("mouseout", function (d, i) {
                        d3.select(this).style('opacity', 1);
                        d3.select(this.parentNode).select('.sales-count').text(function (d, i) { return d3.sum(d.values, function(d) { return d.value; }) + " person today" })
                    })
            })
    ...
    function campaignDonut(element, size) {
        ...
        var data = [
            {
                "browser": "Room 301",
                "icon": "<i class='icon-video-camera2 position-left'></i>",
                "value": 1047,
                "color" : "#66BB6A"
            }, {
                "browser": "Room 302",
    ...
    progressCounter('#goal-progress', 38, 2, "#5C6BC0", 0.82, "icon-trophy3 text-indigo-400", 'Rooms availability', '87% average')
